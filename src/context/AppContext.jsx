...
export function AppProvider({ children }) {
  ...
  const [data, setData]       = useState({
    ...
    incidents: [], certifications: [], uniforms: [], uniformStock: [], observations: [], reviews: [], performance: [], blacklist: [], callIns: []
  });
  ...
  useEffect(() => {
    ...
    const tables = ['employees','applicants','time_off','raises','incidents','certifications','uniforms','uniform_stock','observations','reviews','performance','blacklist','call_ins'];
  ...
  const fetchAll = async () => {
    const [emp, app, to, ra, inc, cert, uni, uniStock, obs, rev, perf, bl, ci] = await Promise.all([
      ...
      supabase.from('observations').select('*').order('obs_date', { ascending: false }),
      ...
      supabase.from('call_ins').select('*').order('date', { ascending: false }),
    ]);
    setData({
      employees:      emp.data  || [],
      applicants:     app.data  || [],
  ...
  const crud = {
    // EMPLOYEES
    addEmployee:    async (e)  => { const { error } = await supabase.from('employees').insert([{ name:e.name, role:e.role, phone:e.phone, email:e.email, start_date:e.start_date||null, birthday:e.birthday, wage:e.wage, avatar:e.avatar, photo_url:e.photo_url||null, active:true, employment_status:e.employment_status||'Good Standing', rehireable:e.rehireable===true?true:e.rehireable===false?false:null }]); if (error) throw error; fetchAll(); },
    updateEmployee: async (e)  => { const { error } = await supabase.from('employees').update({ name:e.name, role:e.role, phone:e.phone, email:e.email, start_date:e.start_date||null, birthday:e.birthday, wage:e.wage, avatar:e.avatar, photo_url:e.photo_url||null, employment_status:e.employment_status||'Good Standing', termination_reason:e.termination_reason||null, termination_date:e.termination_date||null, active:e.active!==undefined?e.active:true, rehireable:e.rehireable===true?true:e.rehireable===false?false:null }).eq('id', e.id); if (error) throw error; fetchAll(); },
  ...
  // ── BLACKLIST ─────────────────────────────────────────────────
  const addCallIn    = async (ci) => { const { error } = await supabase.from('call_ins').insert([{ employee_id:ci.employeeId, date:ci.date, reason:ci.reason, excused:ci.excused===true }]); if (error) throw error; fetchAll(); };
  const updateCallIn = async (ci) => { const { error } = await supabase.from('call_ins').update({ date:ci.date, reason:ci.reason, excused:ci.excused===true }).eq('id', ci.id); if (error) throw error; fetchAll(); };
