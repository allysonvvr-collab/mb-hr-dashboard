...
const EMP_STATUS_STYLE = {
  ...
  'Terminated':    { bg: '#f3f4f6', border: '#e5e7eb', text: '#374151' },
};
...
function Avatar({ name, photoUrl, size=44, grayscale=false }) {
  ...
  if (photoUrl) return <img src={photoUrl} alt={name} style={{ width:size, height:size, borderRadius:'50%', objectFit:'cover', flexShrink:0, border:'2px solid #e5e7eb', filter:grayscale?'grayscale(100%)':'none', opacity:grayscale?0.65:1 }} />;
...
export default function Team() {
  ...
  const [tab, setTab]               = useState('active'); // 'active' | 'terminated' | 'all'
  ...
  const openEdit = (emp) => {
    setForm({
      id: emp.id, name: emp.name||'', role: emp.role||'Crew Worker',
      phone: emp.phone||'', email: emp.email||'',
      start_date: emp.start_date||'', birthday: emp.birthday||'',
      wage: emp.wage!==undefined ? String(emp.wage) : '',
      employment_status: emp.employment_status||'Good Standing',
      termination_reason: emp.termination_reason||'',
      termination_date: emp.termination_date||'',
      rehireable: emp.rehireable??null,
      photo_url: emp.photo_url||'', avatar: emp.avatar||''
    });
  ...
  const save = async () => {
    ...
    if (form.employment_status === 'Terminated' && !form.termination_reason?.trim()) {
      setError('Please enter a reason for termination.'); return;
    }
    ...
    try {
      const emp = {
        ...
        termination_date: form.employment_status === 'Terminated'
          ? (form.termination_date || new Date().toISOString().split('T')[0])
          : null,
        termination_reason: form.employment_status === 'Terminated' ? form.termination_reason : null,
        rehireable: form.employment_status === 'Terminated' ? form.rehireable : null,
      };
  ...
  const baseList = tab === 'active' ? activeEmps : tab === 'terminated' ? terminatedEmps : allEmps;
  ...
  return (
    <div>
      ...
      {/* ── Sub-tabs: Active | Terminated | All ── */}
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:16, flexWrap:'wrap' }}>
        <div style={{ display:'flex', background:'#f3f4f6', borderRadius:10, padding:3, gap:2 }}>
          {[
            ...
            <button key={key} onClick={()=>{ setTab(key); setFilterRole('All'); setSearch(''); }}
              ...
              {label}
              <span style={{ background:tab===key?'#e8f5e9':'#e5e7eb', color:tab===key?'#1B3A2D':'#9ca3af', fontSize:11, fontWeight:700, padding:'1px 6px', borderRadius:8, minWidth:18, textAlign:'center' }}>
                {count}
              </span>
            </button>
          ))}
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center', marginLeft:'auto', flexWrap:'wrap' }}>
          <div style={{ position:'relative' }}>
            ...
            <input style={{ ...inp, paddingLeft:32, width:160, padding:'8px 10px 8px 32px', fontSize:13 }}
      ...
      {/* Role filter chips — active and all tabs only */}
      {tab !== 'terminated' && (
      ...
      <div className="card-grid">
        {filtered.map(emp => {
          ...
          const empStatusStyle = EMP_STATUS_STYLE[emp.employment_status];
            ...
            <div key={emp.id} className="emp-card" style={{ opacity: isTerminated ? 0.88 : 1 }}>
              ...
              <div style={{ display:'flex', gap:10, alignItems:'flex-start', justifyContent:'space-between' }}>
                <div style={{ display:'flex', gap:10, alignItems:'center', minWidth:0, flex:1 }}>
                  ...
                  <div style={{ minWidth:0 }}>
                    ...
                    {/* Role pill */}
                    <span style={{ background:st.bg, border:`1px solid ${st.border}`, color:st.text, fontSize:11, fontWeight:600, padding:'1px 7px', borderRadius:20, display:'inline-flex', alignItems:'center', gap:3, marginTop:3 }}>
                ...
                {/* Right-side badges — Status or Terminated+Rehireable */}
                <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:3, flexShrink:0 }}>
                  {isTerminated ? (
                    <>
                      <span style={{ background:'#f3f4f6', color:'#374151', fontSize:10, fontWeight:700, padding:'3px 8px', borderRadius:20, border:'1px solid #e5e7eb', whiteSpace:'nowrap' }}>
                        Terminated
                      </span>
                      {emp.rehireable === true && (
                        <span style={{ background:'#dcfce7', color:'#166534', fontSize:10, fontWeight:700, padding:'3px 8px', borderRadius:20, border:'1px solid #86efac', whiteSpace:'nowrap' }}>
                          Rehireable
                        </span>
                      )}
                      {emp.rehireable === false && (
                        <span style={{ background:'#fee2e2', color:'#991b1b', fontSize:10, fontWeight:700, padding:'3px 8px', borderRadius:20, border:'1px solid #fca5a5', whiteSpace:'nowrap' }}>
                          Non-Rehireable
                        </span>
                      )}
                    </>
                  ) : (
                    emp.employment_status && emp.employment_status !== 'Good Standing' && (
                      <span style={{ background:empStatusStyle.bg, border:`1px solid ${empStatusStyle.border}`, color:empStatusStyle.text, fontSize:10, fontWeight:700, padding:'3px 8px', borderRadius:20, whiteSpace:'nowrap' }}>
              ...
              <div className="emp-details" style={{ marginTop:10 }}>
                ...
                {isTerminated && emp.termination_date && (
                  <div><span>Terminated</span><span style={{ color:'#6b7280' }}>{emp.termination_date}</span></div>
                )}
                {isTerminated && emp.termination_reason && (
                  <div><span>Reason</span><span style={{ fontSize:12, color:'#374151' }}>{emp.termination_reason}</span></div>
                )}
                {isTerminated && incidentCost > 0 && (
                  <div><span>Incidents</span><span style={{ color:'#dc2626', fontWeight:600 }}>${incidentCost.toLocaleString()} total</span></div>
                )}
              </div>
              ...
              {isAdmin && (
                <div className="card-actions">
                  ...
                  {isTerminated && (
                    <button className="btn-icon" onClick={()=>handleReactivate(emp)} title="Reactivate" style={{ color:'#16a34a' }}><UserCheck size={14}/></button>
                  )}
                  {isTerminated && isAdmin && (
                    <button className="btn-icon" onClick={()=>openEdit(emp)} title="Edit record"><Edit2 size={14}/></button>
                  )}
                  {isSuperAdmin && (
                    <button className="btn-icon danger" onClick={()=>setConfirmDel(emp)} title="Delete permanently"><Trash2 size={14}/></button>
                  )}
                </div>
              )}
            </div>
      ...
      {modal && (
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            {error && (
              <div style={{ background:'#fef2f2', border:'1px solid #fecaca', color:'#dc2626', padding:'10px 12px', borderRadius:8, fontSize:13, marginBottom:12 }}>{error}</div>
            )}
            ...
            <div className="form-grid">
              <label>Full Name
                <input style={inp} value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} placeholder="e.g. John Doe" />
              </label>
              <label>Role
              ...
              <label>Email
                <input style={inp} type="email" value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))} placeholder="email@example.com" />
              </label>
              <label>Start Date
              ...
              {form.role !== 'Owner' && (
                <label>Wage ($/hr)
                  <input style={inp} type="number" step="0.25" min="0" value={form.wage} onChange={e=>setForm(f=>({...f,wage:e.target.value}))} placeholder="15.00" />
                </label>
              )}
              {form.role !== 'Owner' && (
                <label>Status
              ...
              {/* Termination fields — only when Terminated is selected */}
                ...
                <>
                  <label style={{ gridColumn:'1/-1' }}>
                    ...
                    <textarea value={form.termination_reason||''} onChange={e=>setForm(f=>({...f,termination_reason:e.target.value}))} rows={2}
                      style={{ ...inp, resize:'none', marginTop:2 }} placeholder="e.g. No-show, policy violation..." />
                  </label>
                  <label>
                    Rehireable?
        ...
        <div className="modal-overlay" onClick={()=>setConfirmDel(null)}>
          <div className="modal" style={{ maxWidth:360 }} onClick={e=>e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="btn-icon" onClick={()=>setConfirmDel(null)}><X size={18}/></button>
            </div>
            <p style={{ color:'#6b7280', fontSize:14, margin:'8px 0 20px' }}>
              This will permanently delete <strong>{confirmDel.name}</strong> and all their records. Cannot be undone.
            </p>
            <div className="modal-footer">
              <button className="btn-secondary" onClick={()=>setConfirmDel(null)}>Cancel</button>
              <button onClick={()=>{deleteEmployee(confirmDel.id);setConfirmDel(null);}}
