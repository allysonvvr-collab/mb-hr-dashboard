      ...
      <div className="alert-banner" style={{ background:'#f0fdf4', borderColor:'#86efac', color:'#166534' }}>
        ...
        <div>
          ...
          <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginTop:5 }}>
            {(data.employees||[]).filter(e=>e.role!=='Owner'&&e.employment_status!=='Terminated').map(e=>(
            ...
            <div className="form-grid">
              <label>Employee
                <select style={inp} value={form.employeeId} onChange={e => {
                  ...
                  {(data.employees||[]).filter(e=>e.role!=='Owner'&&e.employment_status!=='Terminated').map(e=><option key={e.id} value={e.id}>{e.name} (${Number(e.wage||0).toFixed(2)}/hr)</option>)}
                </select>
              </label>
